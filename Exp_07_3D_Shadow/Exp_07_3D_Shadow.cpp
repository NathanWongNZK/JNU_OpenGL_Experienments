...
GLuint LoadTexture(const char *filename, int width = 128, int height =128)
{ 
    GLuint texture;
    unsigned char *data;
...
void my_Init()							// 用户绘制场景相关的函数在此处定义	
{
	...
	texture[0] = LoadTexture("Data/ground1.bmp");
    texture[1] = LoadTexture("Data/ground2.bmp", 256, 256);
	texture[2] = LoadTexture("Data/wood.bmp", 256, 256);
	texture[3] = LoadTexture("Data/plane.bmp");
